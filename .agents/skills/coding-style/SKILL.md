...
- Name Vitest files `*.spec.test.ts` and use Vitest `expect` assertions. Use Vitest `assert` only when its assertion signature is needed to narrow a value. Name real VS Code extension-host files `*.e2e.test.ts`, colocate them with the extension behavior they cover, and use Chai `expect`. Use Chai `assert` only when its assertion signature is needed to narrow a value. Do not import `node:assert` in test files; this restriction does not apply to test support modules.
